...
class ProjectsDAO {
  ...
  static getProjectById = async id =>
    projects.findOne({ _id: id }, { projection: { projectId: 0 } });
  ...
  static async getUserProjects(projectId) {
    ...
    try {
      ...
      cursor = await projects.find(query).project({ title: 1 });
    } catch (e) {
